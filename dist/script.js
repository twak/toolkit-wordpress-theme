...
/*!
 * JavaScript Cookie v2.1.2 (used for themes selector in docs and on accomodation residence filter)
...
/**
 * acc-select-change.js  
 */

+function ($) {
    'use strict';
    /*
    A simple way to create a select/change/load content.
    The default (non-JS version) will show all content by default
    USAGE: Use the following HTML structure, by adding the class .selectedTexts, the content will be converted to the select/change list

        <div class="selectedTexts">
            <div>
                <h3>Title</h3>
                //content here
            </div>
    ...
    */
    ...
    $(document).on("change", ".select-change-load", function () {
        //get the selected option and the associated text
        var selectedSection = $(this).children('option:selected').val(),
            selectedText = $('#' + selectedSection).html();
        //output the selected text
        $(this).parent('div').next('.select-text').html(selectedText + "<hr />");
    });

    function createSelectText() {
        var options = '';
        $('.select-texts').each(function () {
            ...
            $(this).before("<div class='select-wrapper flag flag-featured island skin-box-module bd-l'><p class='all-caps'>select your residence for your postal adress and how to collect mail and parcels</p><select name='' class='select-change-load'>" + options + "</select></div><div class='select-text'></div>");
            options = ''; //clear the options for the next iteration to prevent being added to the next dynamic select
            $('.select-change-load').trigger('change');
        });
    }
    //get selected text on load
 ...
 * ---------------------
 * Remove Outlines accessibly on click, add when tabbing
 */
 ...
 * body-state.js
 * -------------
 * STATE TOGGLER - toggle body with a class name defined in a data attr
 */
 ...
 * Adds class if content is less than half of its char limit based on attribute
 * Used on the swiper/banner
 */
...
+function ($) {
  'use strict';
    ...
    $('*[data-char-limit]').each(function(){
        ...
        var thisLimit = $(this).data( "char-limit" );
        var thisChar = $(this).text().length;        

        if(thisChar <=(thisLimit/2)){            
            $(this).addClass('char-below-half');
        }    
 ...
 * class-toggler.js
 * ----------------
 * TOGGLER - Toggle an element with a data attr of target and trigger with a class name
 */
...
+(function( $ ) {
	...
	//Custom sorting for timestamps on research opps index, based on https://github.com/filamentgroup/tablesaw#sortable - Advanced Option_: Custom Sort Functions

	// Add a data function to the table header cell
	$( "th.table-saw-custom-timestamp" ).data( "tablesaw-sort", function( ascending ) {
	    // return a function
	    ...
	    return function( a, b ) {	    	
	        ...
	        //Get the data attr of each
	        var dateA = a.element.attributes[0].nodeValue,
	            dateB = b.element.attributes[0].nodeValue;	            
...
+ function ($) {
    ...
    $(document).delegate('*[data-toggle="lightbox"]', 'click', function (event) {
        event.preventDefault();
        $(this).ekkoLightbox({
        	// left_arrow_class: 'glyphicon glyphicon-chevron-left tk-icon tk-icon-chevron-left',
        	// right_arrow_class: 'glyphicon glyphicon-chevron-right tk-icon tk-icon-chevron-right'
        });
...
+function ($) {
  'use strict';
	...
	function createQuickSearch(title, url, cats, type, desc){		
		var quicksearch = '<div id="quicksearch" class="wrapper-lg wrapper-pd wrapper-relative"><div class="quick-search"><ul></ul></div></div>';
...
+ function ($) { 
    ...
    function respTabs(){
        var tabsWidth = $('.tk-tabs-header-resp').width();
        var lisWidth = $('.tk-tabs-header-resp ul').width();

        if(lisWidth > tabsWidth+6){ //if the list items are bigger than ul
            $('.tk-tabs-header-resp').addClass('overflowing'); //add class of overflowing
        } else {
            $('.tk-tabs-header-resp').removeClass('overflowing');
        }  
    }

    $('.tk-tabs-header-resp a').click(function(){ // on click scroll to the a tag  
        var activeLeft = $(this).parent().position().left;
        var listLeft = $('.tk-tabs-header-resp ul .active').closest('ul').position().left;
        
        var offsetLeft =  activeLeft - listLeft;
        $('.tk-tabs-header-resp').animate({scrollLeft: offsetLeft-30}, 100);            
    });

    $('.tk-tabs-header-resp').scroll(function(){   
        
        var tabsLeft = $('.tk-tabs-header-resp').scrollLeft();
...
+ function ($) {
    ...
    /* Idangerous swiper plugin, remove when possible */
    ...
    $(document).ready(function () {
        ...
        //Parallax swiper applies to swiper-2
...
// custom widths at specific sizes

/*
set custom widths like this:
itemsCustom : [
[0, 2],
[450, 4],
[600, 7],
...
try adjusting/passing a variable to adjust the offset
elWidth += base.itemWidth; line 378 in carousel.plugin.js

*/

//$TEUXDEUX refctor oli's code
...
/**
 * Swiper tabs nav (Footer of the swiper tabs)
 */
...
$('.swiper-nav a').on('click', function () {
    ...
    return false;
        ...
        function afterUp(){
          ...
          //console.log(this.reduceRightBy);
        }
